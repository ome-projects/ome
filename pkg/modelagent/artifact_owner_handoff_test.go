...
func TestOrdinaryOwnerHandoffPreservesCleanupAndSharedReferences(t *testing.T) {
	for _, state := range []string{"direct receipt", "shared receipt", "shared child", "legacy parent", "legacy child", "one-sided parent"} {
		t.Run(state, func(t *testing.T) {
			...
			switch state {
			case "direct receipt":
				entry.ArtifactPendingEviction = &ArtifactPendingEviction{ModelUID: task.BaseModel.UID, Path: path}
			case "shared receipt":
