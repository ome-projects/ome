...
type hfArtifactTaskInput struct {
	...
	// ChildModelKey identifies the node ConfigMap entry, e.g. default.basemodel.model-1.
	...
	// ChildModelPath is the model-local symlink path, not the parent directory.
	ChildModelPath string

	// ModelStoreRoot must contain every child path that could use this parent,
	// e.g. /mnt/data/models/customer-model-store. Scans also find child symlinks
...
// validateChildReference permits a child with no reference, or the same parent
// and path. A changed path or inconsistent relationship requires reconciliation.
